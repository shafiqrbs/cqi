        ...
        try{
            if ($this->access == 'allow'){
                ...
                $surveyGroupResults = SurveyResult::where('sur_survey_result.device_id',$input['device_id'])->where('sur_survey_result.survey_id',$survey_id)
                    ->whereBetween('sur_survey_result.created_at', [$start_date, $end_date])
                    ->select([DB::raw('count(sur_survey_result.id) as total'),'sur_survey_result.item_id','sur_item.itemtexten as item_name_en','sur_item.itemtextbn as item_name_bn',DB::raw("CONCAT('0XFF',sur_item.color_code) AS color_code")])
